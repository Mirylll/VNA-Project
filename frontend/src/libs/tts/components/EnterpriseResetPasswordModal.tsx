...
interface EnterpriseResetPasswordModalProps {
  open: boolean;
  enterprise: { id: number; username: string; name: string } | null;
  onClose: () => void;
  onSaved: () => void;
}

export default function EnterpriseResetPasswordModal({
  open,
  enterprise,
  onClose,
  onSaved,
}: EnterpriseResetPasswordModalProps) {
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch(`${baseUrl}/enterprises/${ent.id}`, {
        method: 'PUT',
        headers: {
      ...
      <div className="bg-white rounded-xl shadow-xl w-[420px] overflow-hidden">
        ...
        <div className="p-6">
          <p className="text-sm text-slate-600 mb-4">
            ...
            <span className="font-bold text-slate-800">{ent.username}</span>
            {' - '}
