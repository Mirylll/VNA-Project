...
export default function EnterpriseTypeListPage() {
  ...
  async function handleSave(data: { code: string; name: string; isActive: boolean }) {
    const token = getAuthToken();
    if (!token) return;

    try {
      if (editingItem?.id) {
        await fetch(`${baseUrl}/enterprise-types/${editingItem.id}`, {
          method: 'PUT',
          headers: {
        ...
        await fetch(`${baseUrl}/enterprise-types`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            authorization: `Bearer ${token}`,
  ...
  async function handleDeleteSelected() {
    ...
    const results = await Promise.allSettled(
      selectedIds.map((id) =>
        fetch(`${baseUrl}/enterprise-types/${id}`, {
          method: 'DELETE',
          headers: { authorization: `Bearer ${token}` },
        }),
      ),
    ...
    if (failed > 0) {
      setError(`Đã xoá ${succeeded}/${count} loại hình. ${failed} loại hình không thể xoá.`);
    }
  }
  ...
  return (
    <div className="p-6">
      ...
      <ConfirmDeleteDialog
        open={deleteConfirm}
        message={`Xoá ${selectedIds.length} loại hình đã chọn?`}
